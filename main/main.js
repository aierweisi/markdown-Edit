  ...
  try {
    ...
    if (process.platform.startsWith('win')) {
      ...
      if (
        n.startsWith('\\\\?\\') ||
        n.startsWith('\\\\.\\') ||
        /^[A-Z]:\\\\(?:NUL|CON|PRN|AUX|COM\d|LPT\d)(?:\.|$)/i.test(t)
      )
        return !1
    }
    return !0
...
function sendOpenFile(e) {
  if (e && mainWindow)
    try {
      const n = fs.readFileSync(e, 'utf-8'),
        i = path.basename(e)
      mainWindow.webContents.send('open-file-from-os', { filePath: e, content: n, name: i })
    } catch (e) {
...
async function createWindow() {
  await initStore()
  const { width: e, height: n } = store.get('windowBounds'),
    i = path.join(__dirname, '../assets/icons/icon.ico'),
    a = 'darwin' === process.platform
  ;((mainWindow = new BrowserWindow({
    ...
    trafficLightPosition: a ? { x: 14, y: 16 } : void 0,
    webPreferences: {
    ...
    mainWindow.on('resize', () => {
      ...
      store.set('windowBounds', { width: e, height: n })
    }),
...
function setupTray() {
  ...
  const i = () => {
      mainWindow &&
        (mainWindow.isMinimized() && mainWindow.restore(), mainWindow.show(), mainWindow.focus())
    },
    a = Menu.buildFromTemplate([
      { label: '显示 Markdown Editor', click: i },
      { type: 'separator' },
      {
        label: '退出',
        click: () => {
...
function setupMenu() {
  const e = [
      ...
      {
        label: '模板',
        submenu: [{ label: '模板库', click: () => mainWindow.webContents.send('menu-templates') }],
      },
      { label: '设置', click: () => mainWindow.webContents.send('menu-settings') },
    ],
  ...
  ;(ipcMain.handle('store-get', (e, n) => store.get(n)),
    ...
    ipcMain.handle('file-read', async (e, n) => {
      try {
        ...
        if (!isPathSafe(t)) return { success: !1, error: 'invalid path' }
        return { success: !0, content: fs.readFileSync(t, 'utf-8') }
      } catch (e) {
    ...
    ipcMain.handle('file-save', async (e, n, i) => {
      try {
        ...
        const e = t + '.tmp'
        return (
          await fs.promises.writeFile(e, i, 'utf-8'),
          await fs.promises.rename(e, t),
          { success: !0 }
        )
      } catch (e) {
        return { success: !1, error: e.message }
        ...
        return n && i
          ? t === e
            ...
            : !isPathSafe(t) || !isPathSafe(e)
              ...
              : fs.existsSync(e)
                ? { success: !1, error: 'target exists' }
                : (await fs.promises.rename(t, e), { success: !0, newPath: e })
          : { success: !1, error: 'invalid path' }
      } catch (e) {
        return { success: !1, error: e.message }
      }
    ...
    ipcMain.handle(
      'image-save',
      async (e, { baseDir: n, fileName: i, dataBase64: a, imageDir: t }) => {
        try {
          ...
          ;((s = o
            ? rawDir
            : n
              ? path.join(path.resolve(n), rawDir)
              : path.join(app.getPath('userData'), 'pasted-images')),
            fs.existsSync(s) || fs.mkdirSync(s, { recursive: !0 }))
          let r = i,
            l = path.join(s, r),
            c = 1
          for (; fs.existsSync(l); ) {
            const ext = path.extname(i),
              base = path.basename(i, ext)
            ;((r = `${base}-${c}${ext}`), (l = path.join(s, r)), c++)
          }
          const d = Buffer.from(a, 'base64'),
            u = d[0],
            m = d[1],
            g = d[2],
            p = d[3]
          if (
            !(
              (0x89 === u && 0x50 === m && 0x4e === g && 0x47 === p) ||
              (0xff === u && 0xd8 === m && 0xff === g) ||
              (0x47 === u && 0x49 === m && 0x46 === g) ||
              (0x52 === u &&
                0x49 === m &&
                0x46 === g &&
                0x46 === p &&
            ...
            return { success: !1, error: 'invalid image data' }
          return (
            fs.writeFileSync(l, d),
            {
              success: !0,
              relPath:
                !o && n
                  ? path.relative(n, l).replace(/\\/g, '/')
                  : 'file:///' + l.replace(/\\/g, '/'),
              absPath: l,
            }
          )
    ...
    ipcMain.handle('dialog-save-file', async (e, n) => {
      ...
      return await dialog.showSaveDialog(mainWindow, {
        defaultPath: a,
        filters: n.filters || [{ name: 'Markdown', extensions: ['md'] }],
      })
    }),
    ipcMain.handle(
      'dialog-select-dir',
    ...
    ipcMain.handle('export-pdf', async (e, n) => {
      try {
        ...
        return (fs.writeFileSync(n, e), { success: !0 })
      } catch (e) {
        return { success: !1, error: e.message }
      }
    }),
    ...
    ipcMain.handle('clear-cache', async () => {
      try {
        const { session: e } = require('electron'),
          n = e.defaultSession,
          i = await n.getCacheSize().catch(() => 0)
        return (
          await n.clearCache(),
          await n.clearStorageData({
            storages: [
              'cookies',
              'filesystem',
              'indexdb',
    ...
    ipcMain.handle('focus-window', () => {
      mainWindow &&
        ('win32' === process.platform && mainWindow.blur(),
        mainWindow.focus(),
        mainWindow.webContents.focus())
    }),
    ipcMain.handle('update-titlebar', (e, { color: n, symbolColor: i }) => {
      if (mainWindow && 'win32' === process.platform && mainWindow.setTitleBarOverlay)
        try {
          mainWindow.setTitleBarOverlay({ color: n, symbolColor: i, height: 46 })
        } catch (e) {}
    }),
    ...
    ipcMain.handle(
      ...
      () =>
        !!mainWindow &&
        (mainWindow.isMaximized() ? (mainWindow.unmaximize(), !1) : (mainWindow.maximize(), !0)),
    ),
